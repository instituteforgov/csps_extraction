...
"""
    Purpose
        Extract existing CSPS demographics data and save to database.
    Inputs
        - xlsx: "Demographics working file.xlsx"
            - Collated CSPS demographics data
    Outputs
        - sql: civil_service.civil_service_people_survey_demographics
    Notes
        - Replaces existing data in `civil_service.civil_service_people_survey_demographics`
"""
...
import pandas as pd
from sqlalchemy import DECIMAL, INT
from sqlalchemy.dialects.mssql import NVARCHAR, SMALLINT, UNIQUEIDENTIFIER
...
df_csps.to_sql(
    ...
    name="civil_service_people_survey_demographics",
    con=engine,
    if_exists="replace",
    index=False,
    chunksize=1000,
    dtype={
        ...
        "year": SMALLINT,
        "demographic_variable": NVARCHAR(200),
